...
fun MyArticlesScreen(state: MyArticlesUiState, onSelect: (ArticleStatus) -> Unit, onRefresh: () -> Unit, onBack: () -> Unit, onNew: () -> Unit, onEditArticle: (String) -> Unit, onEditDraft: (String) -> Unit, onDeleteArticle: (String) -> Unit, onDeleteDraft: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        val visibleArticles = state.articles.filter { it.status == state.selectedStatus }
        if ((!state.loading) && visibleArticles.isEmpty() && (state.selectedStatus != ArticleStatus.DRAFT || state.drafts.isEmpty())) EmptyState(stringResource(R.string.articles_empty), Modifier.weight(1f))
