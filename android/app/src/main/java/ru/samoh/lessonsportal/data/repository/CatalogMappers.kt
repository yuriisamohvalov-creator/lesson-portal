...
internal fun ArticleDto.toDomain(isOffline: Boolean = false) = Article(id, title, slug, content, status(status), author?.toDomain() ?: Author("", ""), category.toDomain(), createdAt, publishedAt, rejectionReason, videos.map { it.toDomain() }, isOffline)
